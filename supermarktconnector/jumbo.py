...
BASE_URL = 'https://mobileapi.jumbo.com/v14'
...
class JumboConnector:
    ...
    def get_product_details(self, product):
        ...
        response = requests.get(
            '{}/products/{}'.format(BASE_URL, product_id),
            headers=HEADERS
        )
        if not response.ok:
            response.raise_for_status()
    ...
    def get_store(self, store):
        ...
        response = requests.get(
            '{}/stores/{}'.format(BASE_URL, store_id),
            headers=HEADERS
        )
        if not response.ok:
            response.raise_for_status()
    ...
    def get_all_promotions(self):
        response = requests.get(
            '{}/promotion-overview'.format(BASE_URL),
            headers=HEADERS
        )
        if not response.ok:
            response.raise_for_status()
